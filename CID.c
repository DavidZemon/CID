...
wave dataProcessor (buffer *input, const uint16 in_width, const uint16 in_len) {
	...
	static buffer *hpf2;
	static buffer *hpf3;
	static buffer *intBuf1;
	static buffer *intBuf2;

	// If the positional reset flag is set, clear the HPF and integrator buffers
	if (g_flag_posReset) {
		if (g_flag_POR) { // Allocate memory if Power On Reset...
			hpf1 = (buffer *) malloc(sizeof(buffer));
			hpf2 = (buffer *) malloc(sizeof(buffer));
			hpf3 = (buffer *) malloc(sizeof(buffer));
			intBuf1 = (buffer *) malloc(sizeof(buffer));
			intBuf2 = (buffer *) malloc(sizeof(buffer));
			g_flag_POR = 0;
		}
...
void adc_isr (void) {
	...
	ADCSequenceDataGet(ADC0_BASE + ACCEL_ADC, SEQUENCE, (uint32 *) tempBuffer);
